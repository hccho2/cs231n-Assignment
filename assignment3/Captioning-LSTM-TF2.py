...
class CaptioningRNN:
    ...
    def __init__(self,word_to_idx, input_dim=512, wordvec_dim=128,hidden_dim=128,batch_size = 128,seq_length=1,num_layers=1,variable_scope='foo'):
        ...
        with tf.variable_scope(self.variable_scope,reuse=tf.AUTO_REUSE):
            ...
            self.build() # name scope아래에 있으니, build()내에서 name scope를 줄 필요가 없다.
    def build(self):
...
if Mode==0: 
    with tf.device('/cpu:0'):
        ...
        saver = tf.train.Saver(tf.global_variables()) # 저장할 variable list
    ...
    with tf.Session() as sess:
        batch_size=1000
        sample_model = CaptioningRNN(word_to_idx=data['word_to_idx'],input_dim=input_dim, 
                              wordvec_dim=wordvec_dim,hidden_dim=hidden_dim,batch_size=batch_size,seq_length = 1,num_layers=num_layers,variable_scope='foo1')      

        sample_model2 = CaptioningRNN(word_to_idx=data['word_to_idx'],input_dim=input_dim, 
                              wordvec_dim=wordvec_dim,hidden_dim=hidden_dim,batch_size=batch_size,seq_length = 1,num_layers=num_layers,variable_scope='foo1')      

        
        tf.global_variables_initializer().run()
        saver = tf.train.Saver(tf.global_variables()) # check point에서 복원할 variable list름 넘겨서, saver를 만든다.
        ckpt = tf.train.get_checkpoint_state(ckpt_save_dir)
        max_iteration_chekpoint = get_max_iteration_checkpoint(ckpt)
        if ckpt and max_iteration_chekpoint:
            saver.restore(sess, max_iteration_chekpoint)
